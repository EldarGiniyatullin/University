...
void readWord(MyString &word, Status isCapitalEqualSmall, ifstream &fin)
{
    char symbol;
    fin.get(symbol);
...
bool areEqual(MyString string1, MyString string2)
{
    if (string1.length == string2.length)
    {
        ...
        while (curr1->symbol == curr2->symbol && curr1 != NULL)
        {
            curr1 = curr1->next;
            curr2 = curr2->next;
        }
        if (curr1 == NULL)
        {
            return (curr1->symbol == curr2->symbol) ? true : false;
        }
        else return false;
    }
    else return false;
